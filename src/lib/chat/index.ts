...
// Use explicit index path to avoid type export issues
import type { IChatRoom, IMessage, ChatStatus } from '@/types/index';
  ...
  const channel = supabase
    ...
    .on('presence', { event: 'sync' }, () => {
      ...
      const typingUsers = Object.values(state)
        .flat()
        .filter((user: any) => (user as { is_typing: boolean }).is_typing)
        .map((user: any) => (user as { user_id: string }).user_id);
